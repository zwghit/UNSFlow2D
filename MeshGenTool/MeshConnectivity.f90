...
do i=1,nof
   ...
   !write(21,*)node(p2)%x,node(p2)%y
   !write(21,*)
   x1 = (x2+x1)/2.0d0 ; y1= (y2+y1)/2.0d0
   e1=face(i)%in
   e2=face(i)%out
   if(e1>0) write(4,100)x1,y1,elem(e1)%xc-x1,elem(e1)%yc-y1
   if(e2>0) write(5,100)x1,y1,elem(e2)%xc-x1,elem(e2)%yc-y1
enddo
close(3)
close(4)
close(5)
...
do i=1,noc
   write(3,*)elem(i)%xc,elem(i)%yc
   write(3,*)
enddo
close(3)

open(3,file='tecmesh.dat')
WRITE(3,*) 'VARIABLES="X","Y"'
WRITE(3,*) 'ZONE F=FEPOINT,ET=TRIANGLE'
WRITE(3,*) 'N=',nop,',E=',noc
DO i=1,nop
   WRITE(3,*)node(i)%x,node(i)%y
enddo
DO i=1,noc
   WRITE(3,*)(elem(i)%c2v(j),j=1,elem(i)%nc2v)
END DO
close(3)
...
DO i=1,noc
   do j=1,elem(i)%nc2v
      p1=elem(i)%c2v(j)
      x2 = node(p1)%x ; y2 = node(p1)%y
      write(33,*)x2,y2 
   END DO
      x1 = node(elem(i)%c2v(1))%x ; y1 = node(elem(i)%c2v(1))%y
      write(33,*)x1,y1 
      write(33,*) 
END DO
