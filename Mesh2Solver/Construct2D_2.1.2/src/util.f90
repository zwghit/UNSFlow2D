...
!  Copyright (C) 2013 -- 2015 Daniel Prosser
...
! Write out grid - in reverse i direction to preserve positive volumes
  ...
  if (threed) then
    ...
    !write(iunit,'(es17.8)')                                                    &
    write(iunit,*) (((grid%x(i,j), i=imax,1,-1), j=1,jmax),k=1,1), &
                   (((grid%y(i,j), i=imax,1,-1), j=1,jmax),k=1,1),  &
                   (((   0.d0    , i=imax,1,-1), j=1,jmax),k=1,1)
