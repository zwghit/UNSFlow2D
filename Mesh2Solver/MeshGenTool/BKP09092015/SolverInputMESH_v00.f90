...
type cells  
     ...
     integer(kind=4),allocatable,dimension(:)::fc 
end type cells  
...
!     dxc = xc-x1
!     dyc = yc-y1
!     !cell(in)%cv = cell(in)%cv + 0.5d0*dabs((x1*y2 - x2*y1) + &
!     !              (x2*yc -xc*y2) + (xc*y1 -x1*yc))
!     cell(in)%cv = cell(in)%cv + 0.5d0*dabs(dx*dyc-dy*dxc)
!   endif
!
!   if(out/=0) then
!     xc = cell(out)%xc
...
!     dxc = xc-x1
!     dyc = yc-y1
!     !cell(out)%cv = cell(out)%cv + 0.5d0*dabs((x1*y2 - x2*y1) + &
!     !              (x2*yc -xc*y2) + (xc*y1 -x1*yc))
!     cell(out)%cv = cell(out)%cv + 0.5d0*dabs(dx*dyc-dy*dxc)
!   endif
!enddo
!
!ds=0.0
